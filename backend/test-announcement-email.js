...
async function testAnnouncementEmail() {
  try {
    console.log(
      "Testing announcement email functionality with modern template...\n",
    );
...
Please attend the information session next Friday at 2:00 PM in the main conference room to learn more about these exciting benefits.`,
    ...
    const htmlContent = generateAnnouncementEmail(
      sampleAnnouncement,
      portalUrl,
    );
...
Please log in to the HR Portal to view more details: ${portalUrl}/announcements/${sampleAnnouncement._id}
    ...
    console.error("\nPlease ensure:");
    console.error(
      "1. Google Service Account credentials are properly configured",
    );
    console.error("2. The service account has Gmail API permissions");
    console.error(
      "3. Domain-wide delegation is set up if using Gmail on behalf of another user",
    );
    console.error(
      "4. EMAIL_USER environment variable is set to a valid Gmail address",
    );
  }
}
